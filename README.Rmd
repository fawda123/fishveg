...
dat <- select(fishdat, 
  common.carp_GN, black.bullhead_TN, bluegill_TN, secchim, sdi, phuman, aream2, shedaream2, ecoreg
  ) %>% 
  mutate(
...
# filter by ecoregion
datETF <- filter(dat, ecoreg == 'ETF')
lksETF <- datETF$fish_cat
datETF <- select(datETF, -fish_cat, -ecoreg) %>% 
  decostand(method = 'log', logbase = 10)
datGP <- filter(dat, ecoreg == 'GP')
lksGP <- datGP$fish_cat
datGP <- select(datGP, -fish_cat, -ecoreg) %>% 
  decostand(method = 'log', logbase = 10)

# pca  
pcaETF <- prcomp(datETF) 
pcaGP <- prcomp(datGP)

# tiff('pcaETF.tif', height = 6, width = 8, units = 'in', compression = 'lzw', res = 300, family = 'serif')
ggord(pcaETF, lksETF, vec_ext = 3, size = 3, alpha = 0.8) + 
  ggtitle('ETF')
# dev.off()
# tiff('pcaGP.tif', height = 6, width = 8, units = 'in', compression = 'lzw', res = 300, family = 'serif')
ggord(pcaGP, lksGP, vec_ext = 3, size = 3, alpha = 0.8) + 
  ggtitle('GP')
# dev.off()
```
...
lks <- dat$fish_cat
ecoreg <- dat$ecoreg
cpue_dat <- select(dat, black.bullhead_TN, common.carp_GN) %>%
  decostand(method = 'log', logbase = 10)
env_dat <- select(dat, -black.bullhead_TN, -common.carp_GN, -fish_cat, -ecoreg) %>% 
  decostand(method = 'log', logbase = 10)

# rda
rdamod <- rda(cpue_dat, env_dat)

# tiff('rda_fishcat.tif', height = 6, width = 8, units = 'in', compression = 'lzw', res = 300, family = 'serif')
ggord(rdamod, grp_in = lks, ellipse = F, ptslab = T, addsize = 3, alpha = 0.8, coord_fix = F)
# dev.off()
# tiff('rda_eco.tif', height = 6, width = 8, units = 'in', compression = 'lzw', res = 300, family = 'serif')
# ggord(rdamod, grp_in = ecoreg, ellipse = F, ptslab = T, addsize = 3, alpha = 0.8, coord_fix = F)
# dev.off()
```