...
p1 <- ggord(ppp, grp_in = d$Group, vec_ext = 5, alpha = 0.7, size = 3, txt = 4, arrow = 0.3, repel = T, ellipse = T) + 
  mythm
...
```{r Fig3_carp, fig.height = 6, fig.width = 6}
toscl <- range(d[, c('Carp', 'Bullhead')]) %>% 
  diff
...
rngs <- range(szs) %>% 
  diff %>%
  '/'(toscl) %>% 
  '*'(abs(diff(c(maxrng, minrng)))) %>% 
  '+'(minrng) %>% 
  c(minrng, .)

p <- ggord(ppp, grp_in = d$Group, vec_ext = 5, alpha = 0.7, size = szs, txt = 4, arrow = 0.3, repel = T, ellipse = F, sizelab = 'Carp\nabundance') + 
...
```{r Fig3_bullhead, fig.height = 6, fig.width = 6}
szs <- d$Bullhead
rngs <- range(szs) %>% 
  diff %>%
  '/'(toscl) %>% 
...
p <- ggord(ppp, grp_in = d$Group, vec_ext = 5, alpha = 0.7, size = szs, txt = 4, arrow = 0.3, repel = T, ellipse = F, sizelab = 'Bullhead\nabundance') + 
  scale_size(range = rngs) +
  mythm
