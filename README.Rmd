...
d <- d %>% 
  mutate(
    carp_cat = cut(Carp, c(-Inf, quantile(Carp, c(0.25, 0.75)), Inf), labels = paste0('C ', c('lo', 'md', 'hi'))), 
    bull_cat = cut(Bullhead, c(-Inf, quantile(Bullhead, c(0.25, 0.75)), Inf), labels = paste0('B ', c('lo', 'md', 'hi')))
  ) %>% 
...
p1 <- ggord(ppp, grp_in = d$Group, vec_ext = 5, alpha = 0.7, size = 3, txt = 4, arrow = 0.3, repel = T, ellipse = F) + 
  mythm
